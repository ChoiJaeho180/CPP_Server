...
public:
	...
	static void Init() {
		...
		GPacketHandler[PKT_S_TEST] = [](PacketSessionRef& sessionRef, BYTE* buffer, int32 len) {return HandlePacket<Protocol::S_TEST>(Handle_S_TEST, sessionRef, buffer, len); };
		GPacketHandler[PKT_S_LOGIN] = [](PacketSessionRef& sessionRef, BYTE* buffer, int32 len) {return HandlePacket<Protocol::S_LOGIN>(Handle_S_LOGIN, sessionRef, buffer, len); };
	}
