...
public:
	...
	PacketIterator		operator++(int) { PacketIterator ret = *this; ++_index; return ret; }
private:
	C&			_container;
	uint16		_index;
};
...
public:
	...
	BuffsVictimsList ReserveBuffsVictimsList(BuffsListItem* buffsItem, uint16 victimsCount) {
		uint64* firstBuffsVictimsList = _bw.Reserve<uint64>(victimsCount);
		buffsItem->victimsCount = victimsCount;
		buffsItem->victimsOffset = (uint64)firstBuffsVictimsList - (uint64)_pkt;
		return BuffsVictimsList(firstBuffsVictimsList, victimsCount);
	}
